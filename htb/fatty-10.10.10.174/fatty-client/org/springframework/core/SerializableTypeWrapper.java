...
/*  73 */     return forTypeProvider(new FieldTypeProvider(field));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*  82 */     return forTypeProvider(new MethodParameterTypeProvider(methodParameter));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/*     */     Type type1;
/*  92 */     T t = type;
/*  93 */     while (t instanceof SerializableTypeProxy) {
/*  94 */       type1 = ((SerializableTypeProxy)type).getTypeProvider().getType();
/*     */     }
/*  96 */     return (type1 != null) ? (T)type1 : type;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 106 */     Type providedType = provider.getType();
/* 107 */     if (providedType == null || providedType instanceof Serializable)
/*     */     {
/* 109 */       return providedType;
/*     */     }
/* 111 */     if (GraalDetector.inImageCode() || !Serializable.class.isAssignableFrom(Class.class))
/*     */     {
/*     */       
/* 114 */       return providedType;
/*     */     }
/*     */ 
/*     */     
/* 118 */     Type cached = (Type)cache.get(providedType);
/* 119 */     if (cached != null) {
/* 120 */       return cached;
/*     */     }
/* 122 */     for (Class<?> type : SUPPORTED_SERIALIZABLE_TYPES) {
/* 123 */       if (type.isInstance(providedType)) {
/* 124 */         ClassLoader classLoader = provider.getClass().getClassLoader();
/* 125 */         Class<?>[] interfaces = new Class[] { type, SerializableTypeProxy.class, Serializable.class };
/* 126 */         InvocationHandler handler = new TypeProxyInvocationHandler(provider);
/* 127 */         cached = (Type)Proxy.newProxyInstance(classLoader, interfaces, handler);
/* 128 */         cache.put(providedType, cached);
/* 129 */         return cached;
/*     */       } 
/*     */     } 
/* 132 */     throw new IllegalArgumentException("Unsupported Type class: " + providedType.getClass().getName());
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 188 */       if (method.getName().equals("equals") && args != null) {
/* 189 */         Object other = args[0];
/*     */         
/* 191 */         if (other instanceof Type) {
/* 192 */           other = SerializableTypeWrapper.unwrap((Type)other);
/*     */         }
/* 194 */         return Boolean.valueOf(ObjectUtils.nullSafeEquals(this.provider.getType(), other));
/*     */       } 
/* 196 */       if (method.getName().equals("hashCode")) {
/* 197 */         return Integer.valueOf(ObjectUtils.nullSafeHashCode(this.provider.getType()));
/*     */       }
/* 199 */       if (method.getName().equals("getTypeProvider")) {
/* 200 */         return this.provider;
/*     */       }
/*     */       
/* 203 */       if (Type.class == method.getReturnType() && args == null) {
...
/*     */       }
/* 206 */       if (Type[].class == method.getReturnType() && args == null) {
...
/* Location:              /home/kali/ctf/htb/fatty-10.10.10.174/ftp/fatty-client.jar!/org/springframework/core/SerializableTypeWrapper.class
